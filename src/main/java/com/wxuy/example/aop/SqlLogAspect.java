...
/*    @Autowired  //默认的数据源
    ...
    private Logger logger = LoggerFactory.getLogger(SqlLogAspect.class);
    ...
    @Pointcut("execution(* com.wxuy.example.utils..*.*(..))")  //指向一个奇怪的地方使其失效
//    @Pointcut("execution(* com.wxuy.example.mapper..*.*(..))")
    public void sqlLog() {}
...
/*        JoinPoint的用法
        ...
        logger.info("sql调用方法:{}.{}", joinPoint.getSignature().getDeclaringType().getSimpleName(),joinPoint.getSignature().getName());
    }

    //后置通知
    @AfterReturning(value = "sqlLog()&& @annotation(controllerSqlLog)", returning = "res")
    public void afterReturning(ControllerSqlLog controllerSqlLog ,Object res){
//        System.out.println("这是后置通知！(如果出现异常，将不会调用)！！！！");
    ...
    public Object around(ProceedingJoinPoint pjp,ControllerSqlLog controllerSqlLog) throws Throwable{
